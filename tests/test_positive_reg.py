import pytest
from pages.random_page import RandomEmail
from pages.auth_page import *
from selenium.webdriver.common.by import By
from pages.settings import *

"""Проверка успешного перехода на страницу регистрации"""
@pytest.mark.reg
...
"""Тест проверяет успешную регистрацию в личном кабинете. Для регистрации используется временный адрес электронной почты, 
получаемый в классе RandomEmail с помощью сайта https://www.1secmail.com. Класс получает код для входа на почтовый ящик 
и записывает сгенерированный временный электронный адрес в файл settings.py. В теле класса определяем данные для доступа
к их значениям из всех методов классса."""

@pytest.mark.reg
@pytest.mark.positive
class TestRegistration:
    result_email, status_email = RandomEmail().get_api_email()  # получение временного адреса эл.почты
    valid_email = result_email[0]  # получаем валидный email
